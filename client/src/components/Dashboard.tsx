...
export default function Dashboard() {
  ...
  useEffect(() => {
    loadProjects(); loadCalendar(); loadFeed();
    const t = setInterval(() => { loadProjects(); loadCalendar(); loadFeed(); }, 60000);
  ...
  function pick(id: string) {
    setCapProject(id); setCapMode('running'); setCapSeconds(0);
    if (timerRef.current) clearInterval(timerRef.current);
    timerRef.current = setInterval(() => setCapSeconds(s => s + 1), 1000);
  }
  function onStop() { if (timerRef.current) clearInterval(timerRef.current); setCapMode('noting'); }
  function onDiscard() { if (timerRef.current) clearInterval(timerRef.current); setCapMode('idle'); setCapProject(null); setCapSeconds(0); setCapNote(''); }
  async function onSaveSession() {
    ...
    setFeed(f => [{ project: p?.name || '', note, when: 'Just now', dur: durSec >= 60 ? `${Math.floor(durSec / 60)}m ${durSec % 60}s` : `${durSec}s` }, ...f]);
    setCapMode('idle'); setCapProject(null); setCapSeconds(0); setCapNote('');
    await api.logActivity({ projectId: capProject, note, durationSec: durSec, source: 'live' });
  ...
  async function onAddManual() {
    ...
    const durMatch = /(\d+)/.exec(manDur || '');
    const mins = durMatch ? parseInt(durMatch[1], 10) : 0;
    setFeed(f => [{ project: p?.name || '', note, when: 'Just now', dur: manDur || '—' }, ...f]);
    setManDur(''); setManNote('');
    await api.logActivity({ projectId: manProject, note, durationSec: mins * 60, source: 'manual' });
    loadProjects(); loadFeed();
  }

  const quietProject = projects.find(p => p.quiet);
  const nudge = quietProject
    ? {
    ...
    <div style={rootStyle}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '34px 30px 90px', display: 'flex', flexDirection: 'column', gap: '26px' }}>
        ...
        <div ref={heroRef} style={{ minHeight: '780px' }}>
          {loaded && projects.length > 0 && <Skyline projects={projects} calendarEvents={calendar} onRequestReorder={() => priorityRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' })} />}
        </div>
                  ...
                  <div key={p.id} draggable={!editing}
                    ...
                    {!editing ? (
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '9px' }}>
                        ...
                        <input value={draft?.name ?? ''} onChange={e => setDraft(d => d && { ...d, name: e.target.value })} placeholder="Project name" style={{ ...input, fontSize: '14px', fontWeight: 600 }} />
                        <input value={draft?.blurb ?? ''} onChange={e => setDraft(d => d && { ...d, blurb: e.target.value })} placeholder="What is it? (a short description)" style={input} />
                        <div style={{ display: 'flex', gap: '9px' }}>
                          <input value={draft?.threshold ?? ''} onChange={e => setDraft(d => d && { ...d, threshold: e.target.value })} placeholder="Check-in threshold (e.g. 2 weeks)" style={{ ...input, flex: 1 }} />
                          <input value={draft?.nextStep ?? ''} onChange={e => setDraft(d => d && { ...d, nextStep: e.target.value })} placeholder="Next step (roadmap)" style={{ ...input, flex: 1.4 }} />
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', marginTop: '2px' }}>
                          <div style={{ display: 'flex', gap: '8px' }}>
                            <div onClick={() => removeProject(p.id)} style={{ cursor: 'pointer', fontSize: '12px', fontWeight: 600, color: '#b4453b', padding: '8px 13px', borderRadius: '9px', border: '1px solid #f0d3cf' }}>Remove</div>
        ...
        <div style={{ background: '#fff', border: '1px solid #e2e8f1', borderRadius: '18px', padding: '24px', boxShadow: '0 18px 44px rgba(20,35,58,.06)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '22px', marginTop: '18px' }}>
            ...
            <div style={{ border: '1px solid #e6ecf4', borderRadius: '15px', padding: '18px', background: '#fbfcfe' }}>
              ...
              {capMode === 'noting' && (
                <div style={{ marginTop: '14px' }}>
                  ...
                  <input value={capNote} onChange={e => setCapNote(e.target.value)} placeholder="e.g. Worked through the second verse" style={{ ...input, fontSize: '13px', padding: '10px 12px', borderRadius: '10px' }} />
            ...
            <div style={{ border: '1px solid #e6ecf4', borderRadius: '15px', padding: '18px', background: '#fbfcfe' }}>
              ...
              <div style={{ display: 'flex', gap: '9px', marginTop: '13px' }}>
                <input value={manDur} onChange={e => setManDur(e.target.value)} placeholder="e.g. 45m" style={{ ...input, width: '96px', borderRadius: '10px' }} />
                <input value={manNote} onChange={e => setManNote(e.target.value)} placeholder="What happened?" style={{ ...input, flex: 1, fontSize: '13px', borderRadius: '10px' }} />
              </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', marginTop: '10px' }}>
            {feed.map((f, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '13px', padding: '12px 2px', borderBottom: '1px solid #eef2f7', animation: 'db-in .3s ease both' }}>
